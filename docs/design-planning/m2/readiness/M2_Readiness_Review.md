...
This report preserves the earlier review and its original results. The next two-value experiment is at `../ash-values/`. Storage directions above describe the original delivery, not the current owner decision. The current continuation record is `../../Project_Record.md` and its handoff is `../../handoffs/2026-09-19-planning-publication.md`. The original full snapshot and superseded prepared records remain in the original conversation archive; they are not duplicated here. See the local README for reproduction and provenance boundaries.
