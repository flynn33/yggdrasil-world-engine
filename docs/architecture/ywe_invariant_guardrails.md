...
- `base_world_ontology_invariant`: the nine-coordinate base ontology defines engine-level structural state layers and does not prescribe profile-specific named realms or fictional locations
...
- `paired_resonance_non_morality_invariant`: independently accumulating paired profile-resonance variables are not moral opposites or morality meters
...
- profile-specific embodiments and narrative meanings remain owned by the active reference profile rather than YWE Core
- player-origin and memory-state defaults are supplied by the active profile rather than YWE Core
...
- a reference profile is the engine
